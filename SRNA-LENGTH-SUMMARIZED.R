...
path <- "D:/PROFILING_BY_READ_LENGTH/SUMMARIZE/"

f_list <- list.files(path = path, pattern = ".summarized", full.names = T)

read_df_and_summarise <- function(x) {
  
  recode_to <- c(`mirna` = "A) miRs", `unknown`= "B) Desc",`rrna` = "C) ARNr", `trna` = "D) ARNt", `artifacts` = "E) Artifacts")
  
  LIBRARY_ID <- sapply(strsplit(basename(x), "_"), `[`, 1)
  
  cat("\nReading sample: ", LIBRARY_ID, "\n")
  
  col_names <- c("n", "biotype", "length")
...
out %>% group_by(LIBRARY_ID  ) %>% tally(n)
...
out %>%
  ...
  group_by(Design, Length, rnatype) %>% 
  summarise(n = sum(n)) %>%
  group_by(Design, Length) %>% mutate(pct = n / sum(n)) %>%
  ggplot(aes(x = Length, y = pct, fill = rnatype)) + 
  # facet_grid(group ~ ., scales = "free_y") + 
  geom_col(width = 0.85) +
  ggh4x::facet_nested( Design ~ ., nest_line = F, scales = "free", space = "free") +
...
ggsave(ps, filename = 'PROFILING_SAMPLES_BY_READ.png', path = path, width = 10, height = 5.5, device = png)
